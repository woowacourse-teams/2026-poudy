...
test("shows the workflow link, PR and step progress in the embed", async () => {
  const body = await sendWorkflowRun();
  const embed = body.embeds[0];
  assert.ok(embed);

  // Wiki 알림처럼 워크플로 링크가 본문 맨 앞에 온다.
  ...
  assert.deepEqual(names, ["브랜치", "커밋", "단계", "실행"]);
  assert.equal(fields[2]?.value, "1단계 중 1단계 완료");
  // 첫 시도에는 재시도 회차를 붙이지 않는다.
  assert.equal(fields[3]?.value, "#27 · pull_request");
});
...
test("marks the attempt number only when the run was retried", async () => {
  ...
  assert.equal(fields.at(-1)?.value, "#27 · 재시도 3회차 · pull_request");
});

test("omits the title line when it repeats the workflow name", async () => {
  const body = await sendWorkflowRun({ display_title: "Server CI" });
  const description = body.embeds[0]?.description ?? "";

  assert.equal(description.includes("Server CI\nServer CI"), false);
});
