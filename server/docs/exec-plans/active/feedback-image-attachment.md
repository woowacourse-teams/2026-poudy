...
운영 프로필의 보유기간 작업은 PostgreSQL `received_at` 83일 기준으로 대상을 골라 최종 이미지를
먼저 삭제하고 DB 행을 마지막에 삭제한다. 실패한 행은 다음 주기에 재시도한다.
...
- 예약 83일 삭제 작업이 DB 행·최종 이미지를 모두 지우고 개인정보 처리방침의
