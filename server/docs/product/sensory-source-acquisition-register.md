...
| `source-family:dow-personal-care-formulations` | [Dow Facial Cream 01419](https://www.dow.com/documents/27/27-1/27-1768-01-facial-creamwater-in-oil-emulsion.pdf?iframe=true)은 W/O 유형, phase별 `Wt%`, 공정과 안정성 정보를 가진 공식 PDF다. | 크림 | PDF 표의 시각 대조가 필요하다. 텍스트 추출에서 행·INCI가 어긋날 수 있으며 공식 사용 절차는 별도 확인해야 한다. 재배포 검수도 필요하다. |
...
1. pilot 문서의 최종 HTML/PDF byte를 저장소 밖 원천 저장소에 한 번만 저장한다.
2. 실제 읽은 byte로 `ContentSha256`과 `InputManifest`를 만들고 source revision을 확인한다.
