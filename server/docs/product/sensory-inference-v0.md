...
빈출 성분 보완은 한글명·영문명의 쉼표나 `/`를 분리하지 않고 canonical ingredient ID로만
수행한다. 따라서 `1,2-헥산다이올`의 locant comma와 복수 공식 표기가 포함된 긴 `/` 성분명을
추론 규칙이 자르지 않는다.

첫 후보에서는 역할 태그가 비어 있는 `1,2-헥산다이올`(ID `4840`)과 카프릴릴글라이콜
(ID `2898`)을 곧바로 수분 축 신호로 넣었다. v0.1과 같은 199제품에서 비교하자 140제품의
결과가 바뀌었고, 수분감은 26제품이 한 단계 상승했으며 confidence는 135제품이 상승했다.
2단계 이상 변화와 유분감 변화는 없었지만, 두 성분은 공개 함량이 없고 낮은 함량의 다기능
용도로도 널리 쓰인다. 이 변화가 실제 5분 수분감 개선이라는 근거가 없으므로 후보를 채택하지
않았다. 이 거절 후보의 외부 diff JSON SHA-256은
`f5e44190249cc33d8abdddfd3e732a7f503cb6ce7513ae7499f2e23c1612c978`다.

채택한 `ingredient-role-profile-v0.2`는 기존 v0.1 축 신호를 그대로 유지하고 두 glycol을
`DEFERRED_CONCENTRATION`으로 기록한다. 이 상태에서는 레벨과 confidence 근거로 사용하지 않는다.
v0.1 대비 최종 diff는 수분감·유분감·confidence 모두 199건 unchanged, changed product 0건이며
외부 diff JSON SHA-256은
`2611e941a35a8af5b6e1b35c806237973490b5e66be40a97d3d57bce506ed9b1`다.

준비도 보고서 v3는 보류 성분과 아직 감각 역할·v0 프로필 검토가 없는 성분을 분리한다. 다음
검토 상위에는 정제수, 나이아신아마이드, 아데노신, 알란토인, 트로메타민, 카보머가 남는다.
이들도 이름이나 일반 효능만으로 축 점수를 주지 않고 dry-down·함량·별도 레올로지 채널 또는
감각 근거가 생길 때 검토한다.
