...
제품의 `Ingredients`는 전성분 참조의 입력
순서와 중복을 그대로 보존한다. 둘은 같은 성분을 다루지만 카탈로그 유일성과 제품 배합 순서라는
서로 다른 불변식을 갖는다.
...
큐레이션 공개 조회 모델은 현재 노출할 배너와 상세만 소유한다. 배너 순서는 최상위 배열이,
블록·필터·제품 순서는 각 하위 배열이 소유한다. 블록은 이미지, 일반 제품, 필터 기준 제품
타입으로 나뉘며 각 타입이 자신의 필수 상태와 공개 투영을 소유한다. 일반 제품은 제품 ID만,
...
테이블과 제약은 `db/schema.sql`, 검색 함수·뷰·인덱스는 `db/search.sql`이 소유하며 이 순서로 DB에
한 트랜잭션으로 직접 적용한다. 서버는 `ddl-auto: validate`로
