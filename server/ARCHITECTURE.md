...
3. 첫 용량 표기에서 잘라 낸다. 카탈로그 제품명에는 용량 표기가 한 건도 없으므로 용량 뒤는 전부 기획 문구다. 단위가 붙지 않은 숫자는 자르지 않는다. `글루타샷 10+`와 `SPF50+`는 제품명의 일부다.
4. 용량 없이 붙는 `기획`, `리필` 같은 낱말이 꼬리에 남으면 턴다. 낱말 중간이 아니라 끝에서만 턴다.
...
- 그래도 없으면 뒤 낱말부터 하나씩 덜어 내며 다시 찾는다. 기획명이 제품명 뒤에 붙어 온 공유를 구제한다.

축약에는 하한을 둔다. 두 낱말과 공백을 뺀 네 글자까지만 줄인다. 더 줄이면 남은 낱말이 제품을 가리키지 못한다. 잘못된 제품으로 보내는 것이 못 찾는 것보다 나쁘다.

확정하지 못하면 검색어를 함께 돌려준다. 축약 중 결과가 여러 건 나온 검색어가 있으면 그 검색어를 주고, 없으면 정제한 제품명을 준다. 브랜드 15개와 제품 199개 기준으로 못 찾는 쪽이 기본 경로에 가까우므로, 화면이 빈 결과 대신 볼 것을 갖게 하는 편이 낫다.
