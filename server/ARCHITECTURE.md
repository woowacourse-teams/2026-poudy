...
공유 텍스트 식별은 제품 조회와 성격이 달라 `share` 모듈이 소유한다. `share`는 저장소를 직접 두지 않고 제품·브랜드 조회 포트에 의존한다.

세부 처리 규칙, API 계약, 평가 근거는 [공유 텍스트 매칭](docs/product/share-text-matching.md)에서 관리한다.
...
공유 텍스트 식별 API는 `products`의 공개 인터페이스이며, 내부 구현은 `share` 모듈이 담당한다. 세부 계약은 [공유 텍스트 매칭](docs/product/share-text-matching.md)에서 관리한다.
