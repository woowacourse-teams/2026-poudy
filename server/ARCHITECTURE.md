...
나누거나 제품명 앞부분과 브랜드명 뒷부분으로 나눠 복합 일치를 찾는다. 두 이름의 입력 순서는
상관없다. 브랜드명 부분은 정확 일치나 접두 일치여야 하고 제품명 부분은 기존 이름 검색과 같은
정확·접두·부분 일치를 쓴다. 검색어는 비교 전에 공백을 지우므로 브랜드명과 제품명을 붙여 쓴
입력도 같은 규칙으로 찾는다. 단일 이름 일치는 복합 일치보다 앞에 두고, 복합 일치끼리는 브랜드명과
제품명 일치 등급, 제품 ID 순으로 정렬한다. 제품명 전용 검색은 이 규칙을 사용하지 않는다.
...
2. 남은 링크와 대괄호 홍보 문구를 지운다. 소괄호는 괄호만 벗겨 안의 내용을 남긴 구절과 괄호 전체를 지운 구절을 모두 만든다. 이 작업을 용량보다 먼저 해야 `(120+60+세럼2ml)`의 용량이 잘못된 절단 기준이 되지 않고, `K(+)`처럼 제품 버전을 나타내는 내용도 놓치지 않는다. 단, `[NEW]`가 든 선두 태그는 성분이 다른 이전 제품과 새 제품을 구분하는 버전 표식으로 남긴다.
3. 첫 용량 표기에서 잘라 낸다. 카탈로그 제품명에는 용량 표기가 한 건도 없으므로 용량 뒤는 전부 기획 문구다. 단위가 붙지 않은 숫자는 자르지 않는다. `글루타샷 10+`와 `SPF50+`는 제품명의 일부다. `70ml기획`처럼 단위에 한글이 붙거나 `80mlX3`처럼 배수가 붙어 와도 같은 자리에서 자른다. `40+40ml`, `50+50+15g`처럼 합산된 용량은 첫 번째 숫자부터 자른다. 단위 뒤에 영문이 이어질 때만 용량으로 보지 않는다.
4. 용량 없이 붙는 `기획`, `리필`, `대용량` 같은 낱말이 꼬리에 남으면 턴다. `더블기획`처럼 한 낱말로 붙어 오기도 해서 `기획`으로 끝나는 낱말도 함께 턴다. 기획 낱말 앞에 남은 `1+1` 같은 묶음 수량도 함께 턴다. 낱말 중간이 아니라 끝에서만 턴다.

괄호 내용을 남기거나 지운 구절과 기획 낱말을 털기 전후 구절을 모두 후보로 남기고 넓은 것부터 맞춰 본다. 기획 낱말은 카탈로그 제품명에도 쓰인다. `어성초 크림 카밍 튜브`처럼 이름이 `튜브`로 끝나는 제품이 운영 데이터에 4건 있어, 털어 낸 구절만 쓰면 그 제품을 영영 놓친다.
...
`[NEW]` 버전 표식은 브랜드보다 앞에 오므로 브랜드를 찾을 때만 잠시 건너뛰고 제품명에는 다시 붙인다. 새 버전 이름이 카탈로그에 있으면 이전 버전보다 먼저 확정한다. 새 버전 이름이 없을 때만 표식을 제외하고 기존 제품을 다시 찾는다. 학습 데이터에서 표식을 버렸을 때 토너와 수딩 크림 등 6개 패키지가 전성분이 다른 이전 제품으로 연결됐고, 표식을 보존한 뒤 모두 현재 버전으로 연결됐다.
...
같은 제품에 카탈로그와 다른 라인명이나 성분 표기가 붙는 경우에는 같은 브랜드의 전체 제품 이름을
비교한다. 공백과 대소문자, 선 제품의 `SPF`·`PA`, `pH` 유무, `앤`과 `&` 표기를 정규화한 뒤 글자의 최장
공통 부분 수열과 공통 낱말 비율이 모두 충분한 후보만 남긴다. 마지막 제품 형태와 사용 부위·패키지 범위
표식은 같아야 하고, `토너&크림`처럼 여러 제품 형태가 연결된 세트는 제외한다. 공유 이름에만 `+` 버전
표식이 있거나 `[NEW]`가 새 제품이 아닌 후보에도 연결하지 않는다. 안전 조건을 통과한 후보가 하나이거나
최상위 이름 점수가 차순위보다 충분히 높을 때만 확정한다. 공통 낱말 점수가 높고 차순위와 격차가 큰
후보는 글자 배열이 비슷하기만 한 후보보다 우선한다. 어느 점수도 충분히 벌어지지 않으면 미확정으로 남는다.

현재 리소스 제품 501개를 각 브랜드가 양쪽에 포함되도록 학습용 251개와 검증용 250개로 나눴다. 학습용
제품을 제품명만으로 검색해 상품 ID로 중복 제거한 332건 가운데 실제 전체 공유 텍스트는 331건을 얻었고
1건은 판매가 끝나 수집하지 못했다. 현재 규칙은 수집한 331건 중 272건을 확정하고 하나로 확정하기 어려운
59건은 미확정으로 남긴다. 정제와 이름 비교 규칙을 정하는 데 검증용 제품은 사용하지 않았다.

축약한 검색어로는 확정하지 않는다. 뒤 낱말을 하나만 덜어 내도 다른 제품의 이름에 그대로 들어맞기 때문이다. 초기 실험 당시 운영 카탈로그 199개를 절반으로 갈라 재 봤더니, 카탈로그에 없는 형제 제품을 공유했을 때 학습용 85.0%와 검수용 84.3%가 엉뚱한 제품으로 확정됐다. 축약 확정을 빼면 그 값이 0이 되고 카탈로그에 있는 제품의 확정률은 그대로였다. 잘못된 제품으로 보내는 것이 못 찾는 것보다 나쁘다.

축약은 확정하지 못했을 때 돌려줄 검색어를 고르는 데만 쓴다. 뒤 낱말부터 하나씩 덜어 내며 결과가 나오는 첫 검색어를 주고, 끝내 없으면 정제한 제품명을 준다. 두 낱말과 공백을 뺀 네 글자까지만 줄인다. 초기 브랜드 15개와 제품 199개 기준으로 못 찾는 쪽이 기본 경로에 가까우므로, 화면이 빈 결과 대신 볼 것을 갖게 하는 편이 낫다.
