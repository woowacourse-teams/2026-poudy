...
`ingredient`는 성분, 이름 검색, 근거와 성분에 연결된 태그를 소유한다. `IngredientCatalog`는
ID가 유일한 전체 성분과 검색 인덱스를 소유한다. 제품의 `Ingredients`는 전성분 참조의 입력
...
`Products`는 제품 목록 전체에 대한 검색, 필터, 정렬과 집계를 소유한다. ID별 검색 표현 Map
