...
건을 실어 보내면 그 비용이 화면에 쓰이지 않는다. 상한은 `/api/ingredients/suggestions`에만
걸고, 성분 ID로 하는 조회는 요청한 만큼 전부 돌려준다. 그쪽은 부르는 쪽이 무엇을 몇 개
물었는지 이미 알고 있고, 화면이 담아 둔 성분의 이름을 채우는 데 쓰기 때문이다.
...
`/api/ingredients`는 콤마로 구분한 `ingredientIds`를 필수로 받는다. `IngredientResponse`
목록을 요청한 ID 순서대로 반환하고, 서버 데이터에 없는 ID는 결과에서 제외한다.
...
제안은 입력 중 고를 후보이므로 검색 순서가 가장 좋은 성분을 최대 5건 반환한다.
...
같은 이유로 성분 검색 제안은 페이지가 아니라 상한을 쓴다. 성분은 조건으로 담을 후보를 고르는 자리라 잘 맞는 몇 개면 충분하고, 제품은 그 자체가 목적지다.
