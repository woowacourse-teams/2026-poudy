...
큐레이션 공개 조회 모델은 목록·상세가 공유하는 제목과 설명을 소유한다. 배너는 자체 게시 상태와
썸네일을, 상세는 자체 게시 상태와 블록을 소유한다. `Curation`은 두 구성 요소를 묶어 배너 게시·상세
미게시 조합을 허용하지 않는다. 목록은 배너가 게시된 큐레이션만, 상세는 상세가 게시된
큐레이션만 조회하며 게시 상태 자체는 응답하지 않는다. 배너 순서는 최상위 배열이,
