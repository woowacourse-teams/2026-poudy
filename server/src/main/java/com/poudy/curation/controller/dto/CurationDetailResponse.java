...
public record CurationDetailResponse(
    ...
    @NotNull @Schema(description = "큐레이션 제목") String title,
    @NotNull @Schema(description = "큐레이션 설명") String description,
    @NotNull @Schema(description = "노출 가능한 블록 목록. 저장 순서를 유지하며 빈 배열일 수 있다.") List<CurationBlockResponse> blocks) {

    public static CurationDetailResponse from(ResolvedCurationDetail detail) {
