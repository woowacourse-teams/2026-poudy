...
function copyEntry(sourcePath, destinationPath, activeSources = new Set()) {
  ...
  if (stats.isSymbolicLink()) {
    ...
    // pnpm의 standalone 결과물은 패키지를 .pnpm 저장소의 심볼릭 링크로
    // 만들고, 그 패키지의 의존성은 형제 node_modules에 둔다. 링크 대상
    // 패키지를 실제 디렉터리로 옮기면 중첩된 경우에도 이 형제 디렉터리를
    // 함께 옮겨야 Node.js의 모듈 탐색 경로가 유지된다.
    const isNodeModuleDependency =
      path.basename(path.dirname(destinationPath)) === "node_modules";
    const sourceNodeModules = path.dirname(resolvedSourcePath);

    if (
      isNodeModuleDependency &&
      path.basename(sourceNodeModules) === "node_modules"
    ) {
