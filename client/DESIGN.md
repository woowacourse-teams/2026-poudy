...
- **Layout**: 모든 화면에서 `sticky top-0` 으로 위에 붙는다. `z-30` 이라 바텀시트의 딤(`z-40`)과 시트(`z-50`) 아래에 온다.
- **붙어 있는 것**: 화면의 주제가 스크롤로 사라지는 곳은 그 주제를 바 아래에 함께 붙인다. 브랜드관은 브랜드 축약형(`SummaryHeader`), 카테고리 상세는 같은 분류의 카테고리 칩 줄, 조건 일치 제품은 조건 칩 줄, 카테고리·브랜드는 두 화면을 오가는 탭, 탐색은 검색 방식 탭이다. 제품명 검색은 탭 아래에 검색바도 함께 붙는다. 모두 `z-20` 이고 검색바만 탭 아래인 `z-10` 이다.
- **숨었다 나타나는 것**: 목록을 보는 동안에는 화면을 넓게 쓰도록, 다시 찾을 때만 필요한 줄을 아래로 내리면 숨기고 위로 올리면 돌려놓는다. 제품명 검색의 탭은 숨고 검색바가 그 자리로 올라온다. 조건 일치 제품의 `탐색 조건` 요약은 숨고 칩 줄이 상단바 바로 아래로 붙는다. 숨길 때는 붙는 높이(`top`)를 줄 높이만큼 올려 상단바 뒤로 넣는다. 원래 자리에서는 흐름대로 올라가 사라져 아래 줄과의 사이에 틈이 생기지 않는다. 함께 붙는 줄의 높이는 기기 글자 크기에 따라 달라 재어 쓴다. 키보드로 숨은 탭에 닿으면 다시 내려온다.
- **경계**: 붙은 뒤에만 맨 아래 줄에 `--shadow-sticky-edge` 그림자를 드리운다. 선으로 그으면 흰 목록 위에서 잘 보이지 않고, `border` 는 높이를 1px 늘려 선이 생기는 순간 아래가 밀린다. 검색 방식 탭의 아래 선은 밑줄이 미끄러지는 길이라 늘 긋는다.
- **Motion**: 숨었다 나타나는 줄은 `--transition-duration-travel` 동안 자리를 옮긴다. `prefers-reduced-motion: reduce` 에서는 옮기는 움직임 없이 바로 숨고 나타난다.

### SummaryHeader

- **Structure**: `TopBar variant="sub"` 와 가로 축약형을 한 덩어리로 묶어 화면 위에 붙이는 머리. 제품 상세와 브랜드관이 쓴다. 제품 상세의 축약형은 40px 썸네일, 브랜드·제품명 한 줄, 가장 싼 용량의 가격, 유수분 태그, 저장 버튼으로 이뤄진다. 브랜드관의 축약형은 40px 로고와 브랜드명 한 줄이다.
...
기본 전략은 옅은 tonal shift와 border이며, 떠 있는 메뉴와 앱 프레임, 위에 붙은 줄의 아래 경계(`--shadow-sticky-edge`)에만 shadow를 사용한다. 드롭다운은 흰 배경, `--color-border`, 기존 `shadow-lg`로 주변 목록과 분리한다.
