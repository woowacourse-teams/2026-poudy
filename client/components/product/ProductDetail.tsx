  ...
  return (
    <>
      ...
      <main className="flex-1 pb-10">
        ...
        <section className="flex flex-col items-center gap-3 px-4 pb-6">
          <div className="size-[140px] rounded-2xl bg-surface" />
          <p className="text-[13px] font-medium text-text-secondary">{product.brand.name}</p>
        ...
        <FreeOfCodes codes={product.freeOfCodes} />
      ...
      <ol className="flex flex-wrap items-center gap-1 text-[12px] text-text-secondary">
        {categories.map((path) => (
          <li key={path.id} className="flex items-center gap-1">
            <span>{path.name}</span>
            {path.child ? <span aria-hidden="true">›</span> : null}
            {path.child ? <span>{path.child.name}</span> : null}
          </li>
        ))}
      </ol>
    </nav>
        ...
        {variants.map((variant) => (
          <li key={variant.id} className="flex items-center justify-between px-4 py-3">
            ...
            <span className="text-[13px] text-text-secondary">
              정가 {formatPrice(variant.price)} · {formatVolumeWithUnitPrice(variant.price, variant)}
            </span>
          </li>
        ))}
      </ul>
...
function SkinEffectGroups({ product }: { readonly product: ProductDetailResponse }) {
  ...
  return (
    <section className="border-t-8 border-surface px-4 py-5">
      <h3 className="text-[16px] font-bold text-text-primary">주요 기능</h3>
      <p className="mt-1 text-[13px] text-text-secondary">효과별로 관련 성분을 묶어 보여드려요</p>
...
function FreeOfCodes({ codes }: { readonly codes: ProductDetailResponse["freeOfCodes"] }) {
  if (codes.length === 0) return null;

  return (
    <section className="border-t-8 border-surface px-4 py-5">
      <h3 className="text-[16px] font-bold text-text-primary">성분 정보</h3>

      <ul className="mt-3 grid grid-cols-2 gap-2">
        {codes.map((code) => (
          <li
            key={code}
            className="flex items-center gap-1.5 rounded-lg bg-success-soft px-3 py-2 text-[12px] font-medium text-success"
          >
...
function Ingredients({ ingredients }: { readonly ingredients: ProductDetailResponse["ingredients"] }) {
  return (
    <section className="border-t-8 border-surface px-4 py-5">
      <h3 className="text-[16px] font-bold text-text-primary">전체 성분표</h3>
      <p className="mt-1 text-[13px] text-text-secondary">표기 순서대로 전성분을 보여드려요</p>
      ...
      <ol className="mt-4 flex flex-col">
        {ingredients.map((ingredient, index) => (
          <li key={ingredient.id}>
            ...
            >
              <span className="w-6 font-data text-[12px] text-text-secondary">
                {String(index + 1).padStart(2, "0")}
              </span>
              <span className="flex-1">
                <span className="block text-[14px] text-text-primary">{ingredient.koreanName}</span>
...
function Source({ updatedAt }: { readonly updatedAt: string }) {
  ...
  return (
    <section className="border-t-8 border-surface px-4 py-5">
      <h3 className="text-[14px] font-semibold text-text-primary">상품 정보 출처 안내</h3>
      <p className="mt-1 text-[12px] text-text-secondary">
        브랜드 공식 전성분을 기준으로 정리했어요. 제품 리뉴얼에 따라 실제 표기와 다를 수 있어요.
      </p>
      <p className="mt-2 text-[12px] text-text-secondary">정보 업데이트 · {date}</p>
    </section>
  );
}
