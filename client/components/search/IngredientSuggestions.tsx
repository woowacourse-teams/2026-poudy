...
/**
 * 입력 바로 아래에 겹쳐 뜨는 자동완성 목록.
 *
 * 결과가 화면을 통째로 덮으면 어떻게 빠져나가는지 알기 어렵다. 입력에 붙여 띄우고
 * 높이를 제한해 뒤 화면이 계속 보이게 한다.
 */
  ...
  return (
    <div className="absolute inset-x-0 top-full z-30 mt-1 overflow-hidden rounded-xl border border-[#E8E9EC] bg-white shadow-lg">
      ...
      {items.length === 0 ? (
        ...
        <ul aria-label="성분 검색 결과" className="max-h-[280px] overflow-y-auto">
          {items.map((item) => (
            <li
              key={item.id}
