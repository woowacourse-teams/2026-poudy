          ...
          <li key={category.id}>
            ...
            >
              ...
              <Icon
                name="chevron-down"
                size={16}
                className={`${expanded ? "rotate-180 text-[#555A62]" : "text-[#868B94]"}`}
              />
            </button>
            ...
            {expanded ? (
              <ul>
                {[{ id: 0, name: "전체", productCount: category.productCount }, ...category.children].map((child) => {
                  ...
                  const selected =
                    ids.length > 0 && ids.every((id) => selectedIds.includes(id)) && selectedIds.length === ids.length;
                  ...
                  return (
                    <li key={child.id}>
                      ...
                      >
                        ...
                        <span className={`text-[14px] ${selected ? "font-semibold text-[#212124]" : "text-[#555A62]"}`}>
                          {child.name}
                        </span>
                      </button>
                    </li>
